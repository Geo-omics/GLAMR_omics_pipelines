...
combined_sample_table <- bind_rows(cleaned_up_sample_table, gvhd_samples)
...
bin_info_view <- checkm_pg %>% 
  ...
  left_join(sampinfo_pg %>% select(SampleID, StudyID)) %>% 
  left_join(gtdb_pg %>% select(bin, classification, domain, phylum, class, order, family, genus, species, red_value)) %>% 
  left_join(drep_pg %>% select(bin, secondary_cluster, is_cluster_rep)) %>% 
  show_query()
